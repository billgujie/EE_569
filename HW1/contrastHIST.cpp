...
//void histoD(const unsigned char data[][300], unsigned char *output);
...
//	}
	...
	if (!(file=fopen((savename+"HIST_a.raw").c_str(),"wb"))) {
	...
	savename=argv[2];
	savename.erase(savename.end()-4, savename.end());
	if (!(file=fopen((savename+"HIST_d.raw").c_str(),"wb"))) {
		cout << "Cannot open file: " << argv[2] << endl;
		exit(1);
	}
...
// histogram equal: cumulative probability density method
//int histoC(int input, const int pdf[])
//{
//	int value;
//	for (int i=0;i<=399;i++){
//		for (int j=0;j<=299;j++){
//			value=data[i][j];
//			output[i][j]=(pdf[value]/(400*300))*value;
//		}
//	}
//
//}
...
void PDF(const unsigned char data[][299], int *output)
{
	int temp;
	for (int i=0;i<=399;i++){
		for (int j=0;j<=299;j++){
