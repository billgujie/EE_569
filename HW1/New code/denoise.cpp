...

{
	...
	/*
	 * Method summary: change the following strings to make the program cascade filters
	 *
	 * null: 	do nothing
	 * L3:		average 3x3
	 * L5:		average 5x5
	...
	fwrite(ImagedataF, sizeof(unsigned char), 512*512, file);
	fclose(file);
	return 0;
}
...
void filter(const unsigned char data[][522],  int row, int col, int *output, string mode){//low pass filter
	...
	if (mode=="L3"){ // unweighted 3x3
		...
		int weight[]={1, 4, 7, 4, 1, 4, 16, 26, 16, 4, 7, 26, 41, 26, 7, 4, 16, 26, 16, 4, 1, 4, 7, 4, 1};
		int count=0;
		for (int i=row-2;i<=row+2;i++){
			for (int j=col-2;j<=col+2;j++){
				sum=sum+data[i][j]*weight[count];
